...
module.exports = function(client) {
  function processSingleItem(item, typeOverride) {
    ...
    if (!verbosityFilters.hasOwnProperty(type)) {
      item = omit(item, verbosityFilters.DefaultOmit);
      return item;
    }
    ...
    var isPick = fieldFilterType === 'pick';
    var isOmit = fieldFilterType === 'omit';
    if (!isPick && !isOmit) return item;
    ...
    item = (isPick ? pick : omit)(item, fieldsToFilter);
    return item;
  }
  ...
  return function filterVerboseFields(itemOrItems, url) {
    ...
    if (isArray(itemOrItems)) {
      var typeOverride = last(url.split('/'));
      return map(itemOrItems, function(item) {
