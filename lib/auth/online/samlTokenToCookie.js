...
module.exports = function(client, httpOpts, token, waterfallCb){
  var requestOptions = assign({}, constants.SP_ONLINE_SECURITY_OPTIONS, httpOpts, {
    method: 'post',
    headers : {
  ...
  request.post(requestOptions, function(err, response){
    ...
    if (response.statusCode.toString()[0] !== '3'){
      return waterfallCb(new Error('Received unexpected status code from SP token exchange: ' + response.statusCode));
    }
    ...
    if (!cookies || !cookies.length || !parsedCookies.FedAuth || !parsedCookies.rtFa){
      return waterfallCb(new Error('Could not find auth cookies in sharepoint response'));
    }
